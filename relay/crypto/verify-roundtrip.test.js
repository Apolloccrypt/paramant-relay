...
function makeFrontendV3(mode = 'pdf', opts = {}) {
  ...
  const emailHash = opts.emailHash != null
    ? opts.emailHash
    : crypto.createHash('sha3-256').update(Buffer.from('nick@example.com')).digest('hex');
  ...
  const envelope = mode === 'document'
    ? { ...common, original_filename: 'contract.txt', document_hash: signedHashHex, signer_name: 'Nick' }
