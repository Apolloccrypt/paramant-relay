...
const slh = require("./slhdsa_sha2_192f");

test("SLH-DSA-SHA2-192f keygen produces expected sizes", () => {
...
test("SLH-DSA-SHA2-192f sign/verify roundtrip", () => {
  const { publicKey, secretKey } = slh.generateKeyPair();
  const message = Buffer.from("test message for SLH-DSA-SHA2-192f");
...
test("SLH-DSA-SHA2-192f rejects tampered message", () => {
  const { publicKey, secretKey } = slh.generateKeyPair();
...
test("SLH-DSA-SHA2-192f has no performance_hint (fast variant)", () => {
