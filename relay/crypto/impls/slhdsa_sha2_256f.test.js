...
const slh = require("./slhdsa_sha2_256f");

test("SLH-DSA-SHA2-256f keygen produces expected sizes", () => {
...
test("SLH-DSA-SHA2-256f sign/verify roundtrip", () => {
  const { publicKey, secretKey } = slh.generateKeyPair();
  const message = Buffer.from("test message for SLH-DSA-SHA2-256f");
...
test("SLH-DSA-SHA2-256f rejects tampered message", () => {
  const { publicKey, secretKey } = slh.generateKeyPair();
...
test("SLH-DSA-SHA2-256f has no performance_hint (fast variant)", () => {
