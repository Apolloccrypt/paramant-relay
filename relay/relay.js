...
const server = http.createServer(async (req, res) => {
  ...
  if (path === '/v1' || path.startsWith('/v1/')) {
    ...
    return parasignOpenApi.route({
      ...
      relayIdentity,
      // READ-ONLY gate: reject a /v1 create when the account is already over its
      // monthly signs cap, but do NOT increment here. signs_month counts actual
      // SIGNATURES and is incremented per accepted party-signature in the
      // /v2/envelopes/:id/sign path; counting at create too would double-count a
      // /v1 envelope (its signers post back through that same sign path).
      signQuotaGate: async (accountId, plan) => {
...
<meta property="og:description" content="End-to-end encrypted · Burns after reading · ML-KEM-768">
  ...
  if (req.method === 'POST' && path.startsWith('/v2/envelopes/') && path.endsWith('/sign')) {
    ...
    try {
      ...
      const _signLimit = tiers.tierLimitNum(_signerPlan, 'signs_month');
