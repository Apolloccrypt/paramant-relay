...
async function createEnvelope(deps, apiKey, mode, rec) {
  ...
  if (typeof deps.signQuotaGate === 'function') {
    ...
    if (g && g.allowed === false && g.over_limit) {
      // Same 402 contract as the R018 sign path: one block, at the quota the
      // plan includes. There is no second, higher stop behind it any more.
      const body = { error: 'monthly_sign_quota_reached',
