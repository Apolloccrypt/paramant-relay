...
const CHUNK_PLAIN    = 4.9 * 1024 * 1024; // plaintext bytes per chunk
const PADDED_BLOCK   = 5   * 1024 * 1024; // every upload padded to exactly this size
...
// ── Discover best relay for this API key ──────────────────────────────────────

async function discoverRelay(apiKey, preferredRelay) {
  if (preferredRelay) return preferredRelay.replace(/\/$/, "");
...
browser.cloudFile.onFileUpload.addListener(async (account, { id, name, data }) => {
  ...
  if (!apiKey) throw new Error("No PARAMANT API key configured — open ParaShare FileLink settings.");

  const relay       = await discoverRelay(apiKey, relayUrl);
...
// Burn-on-read: TTL handles cleanup automatically on the relay side
