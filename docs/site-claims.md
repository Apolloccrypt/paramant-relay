...
| 8 | Uptime commitment 99.5 % / 99.95 %; how it is measured | sla, pricing | `sla.html` is the commitment; pricing quoted 99.9 % for Enterprise and now quotes 99.95 %. The measurement paragraph claimed a 60-second probe from multiple EU locations and uptime history in the CT log; neither exists. It now describes exactly what runs: the hourly `product-heartbeat` workflow (`.github/workflows/product-heartbeat.yml`, cron `17 * * * *`) loads paramant.app in a browser (`tests/product-heartbeat.test.mjs`) and pushes a transfer and a signature through `relay.paramant.app` (`tests/transfer-canary.test.mjs`, `tests/parasign-canary.test.mjs`). None of the three calls `GET /health` and the four sector relays are not in the run. `/status` (`frontend/js/status.inline1.js`) is the only thing that fetches `GET /health`, on all five relays, from the visitor's browser. The test fails if the workflow or a canary starts calling `/health` or a sector relay, or if `/status` drops a sector, so the paragraph has to be rewritten when the monitoring changes. | `site-claims.test.mjs` 8 |
