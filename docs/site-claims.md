...
| 20 | "An observer on the network cannot infer/determine file size, type, or content" | security, docs | Finding 5 of `docs/security-audit-2026-04.md`, an accepted trade-off, says the opposite: `total_chunks` travels in the clear (`frontend/js/parashare.page.js`), so the block count of a multi-block transfer is visible and the size follows to an order of magnitude. The site was contradicting the report it publishes. Weakened on both pages | this PR |
...
| 22 | "No third-party requests ... No fonts, CDNs, analytics or pixels" (/parasend), "No tracking." (/pricing) | every public page | The page tree itself: no `script`, `link`, `img`, `iframe`, `source`, `video`, `audio`, `embed`, `object` or `url()` on any public page may point off `paramant.app`. True today and nothing held it there; one shared-header font link would have left both sentences standing | this PR |
