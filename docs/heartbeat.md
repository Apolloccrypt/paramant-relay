...
| `GET /v2/health/deep` with the token | the nine component checks (relay, crypto, storage, memory, disk, tls, users, audit, redis) and their `overall` verdict. The handler answers HTTP 200 even when `overall` is red, so the body is what gets asserted, never the status code |
