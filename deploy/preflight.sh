...
check_port() {
    ...
    if ss -tlnp 2>/dev/null | awk '{print $4}' | grep -q ":${port}$"; then
        ...
        echo -e "${YELLOW}⚠  Port ${port} is in use by ${proc}${NC}"; echo ""
        echo "   Options:"; echo "   1) Use alternate ports (recommended)"; echo "   2) Stop ${proc} manually, then re-run"; echo "   3) Skip (may cause startup failure)"; echo ""
        if [ -t 0 ]; then read -p "   Choose [1/2/3]: " choice; else choice=1; echo "   (non-interactive: auto-selecting option 1)"; fi
        case ${choice:-1} in
            1)
                local alt_http=8080; local alt_https=8443
                while ss -tlnp 2>/dev/null | awk '{print $4}' | grep -q ":${alt_http}$"; do alt_http=$((alt_http + 1)); done
                while ss -tlnp 2>/dev/null | awk '{print $4}' | grep -q ":${alt_https}$"; do alt_https=$((alt_https + 1)); done
...
echo "Checking ports..."
check_port $HTTP_PORT; check_port $HTTPS_PORT; echo ""
...
if [ -n "$DISK_SWAP" ]; then
    ...
    if [[ ! "$disable_swap" =~ ^[Nn]$ ]]; then
        ...
        echo -e "${YELLOW}   Warning: RAM blobs may be paged to disk${NC}"; ISSUES=$((ISSUES + 1))
    fi
...
if [ -f .env ] && grep -q "^ADMIN_TOKEN=.\+" .env; then
    ...
    echo -e "${YELLOW}⚠  ADMIN_TOKEN not set${NC}"; echo ""
    if [ -t 0 ]; then read -p "   Generate a secure ADMIN_TOKEN now? [Y/n]: " gen_token; else gen_token="y"; echo "   (non-interactive: auto-generating token)"; fi
