 ...
 * purpose `app`: fifteen minutes, and the relay accepts it on three routes
 * (POST /v2/billing/checkout, GET /v2/user/history,
 * GET /v2/parasign/audit-export) and refuses it on everything else. It is a
 * DIFFERENT token from the one /parashare holds, not a wider one: neither
 * allowlist contains the other, so giving these pages a credential did not give
 * the send page anything extra. See relay/lib/session-token.js for the lists.
 *
 * WHAT IT DOES NOT DO.
